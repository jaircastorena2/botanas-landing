"use client";

import { motion } from "framer-motion";
import { useInView } from "framer-motion";
import { useRef, useState } from "react";
import Image from "next/image";
import { products, Product } from "@/data/products";
import { useCartStore } from "@/store/cartStore";

function ProductCard({ product, index }: { product: Product; index: number }) {
    const [selectedFlavor, setSelectedFlavor] = useState(product.flavors[0]);
    const [selectedPackIndex, setSelectedPackIndex] = useState(0);
    const [isAdded, setIsAdded] = useState(false);
    const ref = useRef(null);
    const isInView = useInView(ref, { once: true, margin: "-50px" });

    const { addToCart, openCart } = useCartStore();
    const selectedPack = product.packs[selectedPackIndex];

    const handleAddToCart = () => {
        addToCart(product, 1, selectedFlavor, selectedPack);
        setIsAdded(true);
        setTimeout(() => {
            setIsAdded(false);
            openCart();
        }, 600);
    };

    return (
        <motion.div
            ref={ref}
            initial={{ opacity: 0, y: 40 }}
            animate={isInView ? { opacity: 1, y: 0 } : {}}
            transition={{ duration: 0.6, delay: index * 0.15 }}
            className="group"
        >
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 border border-gray-100">
                {/* Product Image */}
                <div className="relative aspect-square bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
                    <Image
                        src={product.image}
                        alt={product.name}
                        fill
                        className="object-contain p-6 group-hover:scale-105 transition-transform duration-500"
                    />

                    {/* Badge */}
                    <div className="absolute top-4 left-4">
                        <span className="bg-white/90 backdrop-blur-sm text-gray-700 text-xs font-medium px-3 py-1.5 rounded-full shadow-sm">
                            {product.netContent}
                        </span>
                    </div>

                    {/* Features overlay on hover */}
                    <motion.div
                        initial={{ opacity: 0 }}
                        whileHover={{ opacity: 1 }}
                        className="absolute inset-0 bg-black/60 flex items-center justify-center p-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                    >
                        <div className="text-center">
                            <ul className="space-y-1">
                                {product.features.slice(0, 4).map((feature) => (
                                    <li key={feature} className="text-white/90 text-sm flex items-center justify-center gap-2">
                                        <svg className="w-4 h-4 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                        </svg>
                                        {feature}
                                    </li>
                                ))}
                            </ul>
                        </div>
                    </motion.div>
                </div>

                {/* Product Info */}
                <div className="p-5">
                    <h3 className="font-serif text-xl text-gray-900 mb-2">
                        {product.name}
                    </h3>
                    <p className="text-sm text-gray-500 leading-relaxed mb-4 line-clamp-2">
                        {product.description}
                    </p>

                    {/* Flavor Selector */}
                    <div className="mb-4">
                        <p className="text-xs font-medium text-gray-600 uppercase tracking-wider mb-2">
                            Sabor
                        </p>
                        <div className="flex flex-wrap gap-1.5">
                            {product.flavors.map((flavor) => (
                                <button
                                    key={flavor}
                                    onClick={() => setSelectedFlavor(flavor)}
                                    className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${selectedFlavor === flavor
                                        ? "bg-gray-900 text-white"
                                        : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                                        }`}
                                >
                                    {flavor}
                                </button>
                            ))}
                        </div>
                    </div>

                    {/* Pack Size Selector */}
                    <div className="mb-5">
                        <p className="text-xs font-medium text-gray-600 uppercase tracking-wider mb-2">
                            Tamaño
                        </p>
                        <div className="flex flex-wrap gap-1.5">
                            {product.packs.map((pack, i) => (
                                <button
                                    key={pack.label}
                                    onClick={() => setSelectedPackIndex(i)}
                                    className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all ${selectedPackIndex === i
                                        ? "bg-gray-900 text-white"
                                        : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                                        }`}
                                >
                                    {pack.label}
                                </button>
                            ))}
                        </div>
                    </div>

                    {/* Price & CTA */}
                    <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                        <div>
                            <span className="text-2xl font-semibold text-gray-900">
                                ${selectedPack.price}
                            </span>
                            <span className="text-sm text-gray-400 ml-1">MXN</span>
                        </div>
                        <motion.button
                            onClick={handleAddToCart}
                            disabled={isAdded}
                            className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all ${isAdded
                                ? "bg-green-500 text-white"
                                : "bg-gray-900 text-white hover:bg-gray-800"
                                }`}
                            whileTap={{ scale: 0.95 }}
                        >
                            {isAdded ? "✓ Agregado" : "Agregar"}
                        </motion.button>
                    </div>
                </div>
            </div>
        </motion.div>
    );
}

export default function ProductGrid() {
    const ref = useRef(null);
    const isInView = useInView(ref, { once: true, margin: "-100px" });

    return (
        <section id="productos" ref={ref} className="py-20 md:py-28 bg-gray-50">
            <div className="container-luxury">
                {/* Section Header */}
                <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={isInView ? { opacity: 1, y: 0 } : {}}
                    transition={{ duration: 0.8 }}
                    className="text-center mb-14"
                >
                    <p className="text-sm font-medium text-gray-500 uppercase tracking-[0.2em] mb-3">
                        Nuestra Colección
                    </p>
                    <h2 className="font-serif text-3xl md:text-4xl lg:text-5xl text-gray-900 mb-4">
                        Snacks Premium
                    </h2>
                    <p className="text-gray-600 max-w-2xl mx-auto">
                        Elige tu sabor favorito y el tamaño ideal para ti.
                        Todos nuestros productos son libres de grasas trans y autorizados para escuelas.
                    </p>
                </motion.div>

                {/* Products Grid - 2 columns on mobile, 3 on desktop */}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    {products.map((product, index) => (
                        <ProductCard key={product.id} product={product} index={index} />
                    ))}
                </div>

                {/* Mayoreo CTA */}
                <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={isInView ? { opacity: 1, y: 0 } : {}}
                    transition={{ duration: 0.8, delay: 0.5 }}
                    className="mt-16 bg-gradient-to-r from-gray-900 to-gray-800 rounded-2xl p-8 md:p-12 text-center"
                >
                    <h3 className="font-serif text-2xl md:text-3xl text-white mb-4">
                        ¿Buscas mayoreo o marca propia?
                    </h3>
                    <p className="text-gray-300 mb-6 max-w-xl mx-auto">
                        Ofrecemos precios especiales para distribuidores y fabricamos con tu marca.
                        Contáctanos para cotización personalizada.
                    </p>
                    <a
                        href="https://wa.me/5213328286378?text=Hola, me interesa información sobre mayoreo y marca propia"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-2 bg-white text-gray-900 px-6 py-3 rounded-full font-medium hover:bg-gray-100 transition-colors"
                    >
                        <svg className="w-5 h-5 text-green-600" fill="currentColor" viewBox="0 0 24 24">
                            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                        </svg>
                        Contactar por WhatsApp
                    </a>
                </motion.div>
            </div>
        </section>
    );
}
